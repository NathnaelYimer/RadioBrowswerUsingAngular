...
export class NewstationComponent implements AfterViewInit {
  ...
  add_language(language) {
    ...
    const index = this.station.language_arr.indexOf(language);
    if (index < 0) {
  ...
  add_tag(tag) {
    ...
    const index = this.station.tags_arr.indexOf(tag);
    if (index < 0) {
  ...
  onSubmit(content) {
    this.sending = true;
    const station_post: DataStationNew = {
    ...
    this.rb.addStation(station_post).toPromise().then(() => {
